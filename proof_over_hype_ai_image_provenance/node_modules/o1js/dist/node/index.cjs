  ...
  function toProver(key, i) {
    async function prove_(publicInput, ...args) {
      ...
      if (!doProving) {
        ...
        let publicOutput2 = await methods[key].method(publicInput, previousProofs2);
        return ProgramProof.dummy(publicInput, publicOutput2, maxProofsVerified);
      }
...
function picklesRuleFromFunction(publicInputType, publicOutputType, func, proofSystemTag, { methodName, args }, gates) {
...
var UnfinishedForest = class _UnfinishedForest {
  ...
  constructor(mutable, final) {
    assert2(final === void 0 !== (mutable === void 0), "final or mutable");
    this.final = final;
  ...
  constructor(address, tokenId) {
    ...
    this.sender = {
      ...
      getAndRequireSignatureV2() {
        let sender2 = this.getUnconstrainedV2();
        AccountUpdate3.createSigned(sender2);
        return sender2;
      }
    };
...
function encryptBytes(message, otherPublicKey) {
  ...
  return {
    ...encryptV2(chunks.map((chunk2) => bytesToWord(chunk2)), otherPublicKey),
    messageLength
  };
}
