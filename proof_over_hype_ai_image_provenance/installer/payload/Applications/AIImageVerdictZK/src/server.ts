...
function extractImageUrls(html: string, baseUrl: string, maxImages = 5): string[] {
...
async function generateProof(imageHash: Field, verdict: boolean) {
...
async function buildUnsignedTx(
  payload: { imageHash: string; verdict: boolean; oraclePublicKey: string; signature: unknown },
  feePayer: string
) {
  if (!process.env.ZEKO_GRAPHQL) {
    throw new Error('ZEKO_GRAPHQL env var not set');
