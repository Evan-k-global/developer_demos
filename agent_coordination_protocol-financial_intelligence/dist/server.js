...
import { Bool, AccountUpdate, Field, Mina, PrivateKey, PublicKey, Signature, UInt64, UInt32, Encoding, MerkleTree, Poseidon, fetchAccount, fetchTransactionStatus } from 'o1js';
...
const massiveFlatfilesStocksPrefix = process.env.MASSIVE_FLATFILES_STOCKS_PREFIX || 'us_stocks_sip/day_aggs_v1';
const massiveFlatfilesCryptoPrefix = process.env.MASSIVE_FLATFILES_CRYPTO_PREFIX || 'global_crypto/day_aggs_v1';
...
let cachedOutputKey = null;
async function getOutputEncryptionKey() {
    if (cachedOutputKey)
...
function verifySignedMessage(publicKeyBase58, signatureBase58, message) {
...
const tzekoTokenAddress = process.env.TZEKO_TOKEN_ADDRESS ?? 'B62qjUhPDbMskxMduyzkyGnK6LZwHksuuYPRjyF4owJM7UWLGJynN36';
app.use(cors());
...
function normalizeMassiveSymbol(rawSymbol) {
...
function inferMassiveColumns(headers) {
    const normalized = headers.map((h) => h.trim().toLowerCase());
    const symbolIndex = normalized.findIndex((h) => ['ticker', 'symbol', 'sym', 's'].includes(h));
    const closeIndex = normalized.findIndex((h) => ['close', 'c', 'close_price', 'closeprice'].includes(h));
...
function containsProhibitedPersonalContext(text) {
...
function sanitizeTextOutput(value) {
    let cleaned = value;
    cleaned = cleaned.replace(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, '[REDACTED_EMAIL]');
    cleaned = cleaned.replace(/\b(\+?\d[\d\s().-]{7,}\d)\b/g, '[REDACTED_PHONE]');
    cleaned = cleaned.replace(/\b(I am|I'm|My name is)\s+[A-Za-z'-]{2,}\b/gi, '[REDACTED_NAME]');
...
function sanitizeModelOutput(output) {
    ...
    if (typeof output === 'string')
        return sanitizeTextOutput(output);
    if (Array.isArray(output))
        return output.map((item) => sanitizeModelOutput(item));
...
function normalizeActionLabel(action) {
    ...
    if (value === 'BUY' || value === 'BULLISH' || value === 'POSITIVE')
        return 'POSITIVE';
...
async function readCreditsLedger() {
    return readJson(creditsLedgerPath, {
        balances: {},
        pending: {},
        slashes: [],
        confirms: []
...
async function readNullifierSet() {
    return readJson(nullifierSetPath, { nullifiers: {} });
}
async function writeNullifierSet(payload) {
...
async function fetchSymbolIndex() {
    ...
    if (!lines.length)
        throw new Error('Symbol index empty');
...
async function ensureSymbolIndexFresh() {
    try {
        const cached = await readJson(symbolIndexPath, {});
...
function pickTickersFromNames(prompt, max = 3) {
    if (!nameIndexCache)
        return [];
...
async function uploadEncryptedToIpfs(payload) {
    if (!ipfsApiUrl)
        return null;
    try {
...
function createMassiveS3Client() {
    if (!massiveS3AccessKeyId || !massiveS3SecretAccessKey)
        return null;
...
async function listLatestMassiveFlatfileKey(client, prefix) {
    const now = new Date();
    try {
        const candidates = [];
        for (let offset = 0; offset < 2; offset += 1) {
            ...
            const keys = list.Contents?.map((obj) => obj.Key).filter((key) => Boolean(key)) ?? [];
...
async function updateFlatfileSeries(symbol, date, close) {
...
async function readFlatfileSeries(symbol) {
    try {
...
async function getMassiveTargetSymbols() {
    ...
    try {
        const sp500 = await readJson(sp500Path, {});
    ...
    try {
        const macro = await readJson(macroPath, {});
    ...
    try {
        const cryptoList = await readJson(cryptoPath, {});
...
async function syncMassiveFlatfile(prefix, label, isCrypto) {
    ...
    if (!latestKey)
        return { label, ok: false, reason: 'no files found' };
    const match = latestKey.match(/(\d{4})\/(\d{2})\/(\d{4}-\d{2}-\d{2})/);
    const fileDate = match?.[3];
    if (!fileDate)
        return { label, ok: false, reason: 'unable to parse date' };
    const state = await readJson(massiveFlatfilesStatePath, {});
    ...
    for await (const line of rl) {
        ...
        if (!columns || columns.symbolIndex < 0 || columns.closeIndex < 0)
            continue;
        ...
        if (!symbol || !Number.isFinite(closeValue))
            continue;
        if (targets && !targets.has(symbol))
            continue;
...
async function ensureMerkleState() {
    const current = await readJson(merklePath, {
        height: merkleHeight,
        nextIndex: 0,
        leaves: []
    });
...
async function ensureOutputMerkleState() {
    const current = await readJson(outputMerklePath, {
        height: merkleHeight,
        nextIndex: 0,
        leaves: []
    });
...
async function ensureAgentMerkleState() {
    const current = await readJson(agentMerklePath, {
        height: merkleHeight,
        nextIndex: 0,
        leaves: []
    });
...
async function ensureCreditsMerkleState() {
    const current = await readJson(creditsMerklePath, {
        height: merkleHeight,
        nextIndex: 0,
        leaves: []
    });
...
async function ensureNullifierMerkleState() {
    const current = await readJson(nullifierMerklePath, {
        height: merkleHeight,
        nextIndex: 0,
        leaves: []
    });
...
async function commitLeaf(leaf) {
...
async function commitOutputLeaf(leaf) {
...
async function commitAgentLeaf(leaf) {
...
async function commitCreditsLeaf(leaf) {
...
async function commitNullifierLeaf(leaf) {
...
function getOracleKey() {
    ...
    if (env)
        return PrivateKey.fromBase58(env);
...
function computeLeaf(requestHash, agentIdHash) {
    return Poseidon.hash([requestHash, agentIdHash]);
}
function computeOutputLeaf(requestHash, outputHash) {
    return Poseidon.hash([requestHash, outputHash]);
}
function computeAgentLeaf(agentIdHash, ownerHash, treasuryHash, stakeAmount) {
    return Poseidon.hash([agentIdHash, ownerHash, treasuryHash, stakeAmount]);
}
function computeCreditsLeaf(ownerHash, amount, direction) {
    return Poseidon.hash([ownerHash, amount, direction]);
}
function computeNullifierLeaf(ownerHash, nonceHash) {
    return Poseidon.hash([ownerHash, nonceHash]);
}
function signCreditsUpdate(creditsRoot, nullifierRoot, spendAmount, platformAmount) {
...
async function buildUnsignedOutputTx(payload, feePayer) {
    const start = debugTxTiming ? Date.now() : 0;
    const network = getNetwork();
    if (!network.graphql) {
        throw new Error('ZEKO_GRAPHQL env var not set');
...
async function buildUnsignedCreditsTx(payload, feePayer) {
    const network = getNetwork();
    if (!network.graphql) {
        throw new Error('ZEKO_GRAPHQL env var not set');
    }
    ...
    const tx = await Mina.transaction({ sender: feePayerPk, fee }, async () => {
        ...
        if (spendTo && spendAmountMina > 0) {
            ...
            await zkapp.submitSignedCreditsSpend(creditsRoot, nullifierRoot, oraclePk, signature, spendTo, spendAmount, platformPk, platformAmount);
        }
        else {
...
function simulateReturn(symbol, timestamp, action) {
...
function computePerformance(outputs) {
    if (!outputs.length) {
        return { avgReturn: 0, winRate: 0, cagr: 0 };
    }
    const returns = outputs.map((entry) => simulateReturn(entry.symbol, entry.fulfilledAt, entry.action));
...
async function readEdgarCache(key, maxAgeMs = 24 * 60 * 60 * 1000) {
    try {
...
async function writeEdgarCache(key, payload) {
...
async function getTickerCikMap() {
    ...
    const cached = await readEdgarCache(cacheKey);
    const data = cached ||
        (await fetchSecJson('https://www.sec.gov/files/company_tickers.json'));
    if (!cached)
...
async function getCompanyFacts(cik) {
    ...
    if (cached)
        return cached;
    const data = await fetchSecJson(`https://data.sec.gov/api/xbrl/companyfacts/CIK${cik}.json`);
    await writeEdgarCache(cacheKey, data);
    return data;
}
async function getCompanySubmissions(cik) {
    ...
    if (cached)
        return cached;
    const data = await fetchSecJson(`https://data.sec.gov/submissions/CIK${cik}.json`);
    await writeEdgarCache(cacheKey, data);
    return data;
}
...
function buildLatestFiling(submissions) {
    const recent = submissions?.filings?.recent;
    if (!recent)
        return { type: '10-Q', date: 'n/a', summary: 'No recent filing data.' };
    const forms = recent.form || [];
    const dates = recent.filingDate || [];
    const index = forms.findIndex((form) => form === '10-K' || form === '10-Q');
    if (index === -1)
        return { type: forms[0] || '10-Q', date: dates[0] || 'n/a', summary: 'Recent filing.' };
    return {
        type: forms[index],
...
async function fetchStooqDaily(symbol) {
    ...
    const tryFetch = async (baseUrl, ticker) => {
    ...
    if (altPrimary.length)
        return altPrimary;
    const altFallback = await tryFetch('https://stooq.pl/q/d/l/', sym);
    return altFallback;
}
const priceCache = new Map();
let cryptoSymbolCache = null;
async function isCryptoSymbol(symbol) {
    if (!cryptoSymbolCache) {
        try {
            const data = await readJson(cryptoListPath, { symbols: [] });
...
async function readPriceCache(symbol) {
    try {
...
async function writePriceCache(symbol, series) {
...
async function fetchAlphaVantageDaily(symbol, isCrypto) {
    if (!alphaVantageKey)
        return [];
    const url = isCrypto
        ? `https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol=${encodeURIComponent(symbol)}&market=USD&apikey=${alphaVantageKey}`
        : `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${encodeURIComponent(symbol)}&apikey=${alphaVantageKey}&outputsize=compact`;
    const res = await fetch(url);
    const text = await res.text();
...
async function fetchTwelveDataDaily(symbol, isCrypto) {
    if (!twelveDataKey)
        return [];
    const symbolParam = isCrypto ? `${symbol}/USD` : symbol;
    const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbolParam)}&interval=1day&outputsize=200&apikey=${twelveDataKey}`;
    const res = await fetch(url);
    const text = await res.text();
    if (process.env.DEBUG_PRICE === 'true') {
...
function normalizeMassiveTicker(symbol) {
    if (symbol === 'BRK-B')
        return 'BRK.B';
    if (symbol === 'BRK-A')
        return 'BRK.A';
...
async function fetchMassiveDaily(symbol, isCrypto) {
    if (!massiveApiKey)
        return [];
    ...
    const url = `https://api.massive.com/v2/aggs/ticker/${encodeURIComponent(massiveTicker)}/range/1/day/${from}/${to}?adjusted=true&sort=asc&limit=5000&apiKey=${massiveApiKey}`;
    const res = await fetch(url, {
        headers: {
...
async function fetchBestPriceSeries(symbol) {
...
function shouldUseAlphaVantage() {
    const now = Date.now();
    if (now - lastAlphaRequestAt < 1100)
        return false;
...
function shouldUseMassive() {
    const now = Date.now();
    if (now - lastMassiveRequestAt < 600)
        return false;
    lastMassiveRequestAt = now;
    return true;
}
async function fetchProviderDebugMassive(symbol, isCrypto) {
    if (!massiveApiKey)
...
async function fetchProviderDebugAlpha(symbol, isCrypto) {
    if (!alphaVantageKey)
        return { ok: false, reason: 'missing key' };
    const url = isCrypto
        ? `https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol=${encodeURIComponent(symbol)}&market=USD&apikey=${alphaVantageKey}`
...
async function fetchProviderDebugTwelve(symbol, isCrypto) {
    if (!twelveDataKey)
        return { ok: false, reason: 'missing key' };
    const symbolParam = isCrypto ? `${symbol}/USD` : symbol;
    const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbolParam)}&interval=1day&outputsize=200&apikey=${twelveDataKey}`;
...
function pickExitDate(series, entryDate) {
...
async function computeRealizedPnL(outputs, includeDebug = false) {
    if (!outputs.length) {
        return includeDebug
            ? { avgReturn: 0, winRate: 0, cagr: 0, coverage: 0, totalDays: 0, debug: [] }
    ...
    for (const [symbol, signals] of bySymbol.entries()) {
        try {
            ...
            for (let i = 0; i < sorted.length; i += 1) {
                ...
                if (signal.action === 'POSITIVE')
                    position = 'LONG';
                else if (signal.action === 'NEGATIVE')
                    position = 'SHORT';
                else if (signal.action === 'NEUTRAL')
                    position = 'FLAT';
                lastIndex = idx;
            }
...
app.get('/api/market', async (_req, res) => {
    try {
        ...
        const equities = await Promise.all(symbols.map(async (symbol) => {
...
app.get('/api/validate-models', async (_req, res) => {
    try {
        ...
        for (const check of checks) {
            ...
            const outputs = Array.isArray(output?.outputs) ? output.outputs : [output];
            const symbols = outputs.map((entry) => entry?.symbol).filter(Boolean);
            const priceSeries = await Promise.all(symbols.map(async (symbol) => {
...
app.get('/api/supported-symbols', async (_req, res) => {
    const edgar = await readJson(edgarPath, {});
    const sp500 = await readJson(sp500Path, {
        tickers: []
    });
    const crypto = await readJson(cryptoPath, { symbols: [] });
    ...
    const coverage = await Promise.all(symbols.slice(0, 200).map(async (symbol) => {
...
app.get('/api/price-coverage', async (_req, res) => {
    ...
    const requestsStore = await readJson(requestsPath, { requests: [] });
    const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;
    const coverage = await Promise.all(data.agents.map(async (agent) => {
...
function pickTickersFromPrompt(prompt, tickers, allowUnknown = false) {
    const aliasMap = {
        GOOG: 'GOOGL',
        'BRK.B': 'BRK-B',
        'BRK/A': 'BRK-A',
...
function pickTopEdgarSymbols(edgarData, limit = 3) {
...
function classifyAction(score, band = 0.12) {
    if (score >= 0.5 + band)
        return 'POSITIVE';
    if (score <= 0.5 - band)
        return 'NEGATIVE';
...
async function simulateModel({ agentId, prompt, requestId }) {
    const edgar = await readJson(edgarPath, {});
    const sp500 = await readJson(sp500Path, {
        tickers: []
    });
    ...
    const rankingPrompt = /(highest|top|best|rank|strongest|most bullish|highest confidence|best pick|top pick)/i.test(prompt);
    const lowestPrompt = /(lowest|worst|bottom|weakest|most bearish|lowest confidence|worst pick|sell)/i.test(prompt);
    const requested = pickTickersFromPrompt(prompt, agentId === 'crypto-quant' ? cryptoUniverse : universe, true);
        ...
        : agentId === 'crypto-quant'
            ...
            : commodityKey
                ...
                : thematicSymbols.length
                    ? thematicSymbols
                    : universe.slice(0, 3);
    const scoreRanked = (direction = 'top') => {
    ...
    const selectedSymbols = shouldRank
        ...
        : agentId === 'edgar-scout'
            ...
            : symbols;
    const outputs = await Promise.all(selectedSymbols.map(async (symbol) => {
        const seed = `${requestId}:${agentId}:${symbol}`;
        const baseScore = seededScore(seed);
        const filing = edgar[symbol]?.latestFiling ?? { type: '10-Q', date: '2025-10-30', summary: 'No filing data loaded.' };
        const highlights = edgar[symbol]?.highlights ?? ['No EDGAR highlights loaded.'];
        let priceMomentum = 0.5;
        let lookbackReturn = null;
        ...
        if (agentId === 'edgar-scout') {
            ...
            try {
                ...
                if (cik) {
                    ...
                    if (incomeYoY !== null)
                        profitGrowth = incomeYoY;
                    if (revenueYoY !== null)
                        revenueGrowth = revenueYoY;
            ...
            if (!hasSecData) {
                try {
                    const sample = await readJson(edgarPath, {});
            ...
            if (!hasSecData || !hasPrice) {
                ...
                const cached = await readEdgarCache(cachedKey, Infinity);
                if (cached) {
                    return cached;
                }
                const missing = [];
                if (!hasSecData)
                    missing.push('SEC data');
                if (!hasPrice)
                    missing.push('price series');
        ...
        const sector = sp500.tickers.find((entry) => entry.symbol === symbol)?.sector ??
...
async function callExternalModel(agent, payload) {
...
function validateModelResponse(payload) {
    const normalized = payload && typeof payload === 'object' && Array.isArray(payload.outputs)
        ? payload
        : { outputs: [payload] };
...
app.get('/api/leaderboard', async (_req, res) => {
    ...
    const requestsStore = await readJson(requestsPath, { requests: [] });
    const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;
    const stats = await Promise.all(data.agents.map(async (agent) => {
        const agentRequests = requestsStore.requests.filter((req) => req.agentId === agent.id);
...
app.post('/api/agent-intent', async (req, res) => {
    try {
        const { name, tagline, priceMina, description, ownerPublicKey, treasuryPublicKey, modelEndpoint, modelAuth, hostingType } = req.body ?? {};
        if (!name || !tagline)
...
app.post('/api/credits/spend-intent', async (req, res) => {
    try {
        ...
        const { oraclePk, signature } = signCreditsUpdate(creditsCommit.newRoot, nullifierCommit.newRoot, amountField, Field.from(Math.round(platformFee * 1e9)));
...
app.get('/api/edgar', async (req, res) => {
    const symbol = String(req.query.symbol || '').toUpperCase();
    const data = await readJson(edgarPath, {});
